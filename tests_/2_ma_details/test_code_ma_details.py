...
from pages.ma_details_files.download_data.data_about_people import *
...
def test_code_data_about_people():
    generate_data_about_people(con, engine, True)
    data_ma_detail = download_data_about_people(con, engine, True)
...
def test_add_prefix():
    ...
    path_to_csv = os.path.abspath(
        os.path.join(os.path.dirname(__file__), '../../pages/ma_details_files/tmp_file/people_camp.csv'))
    data_to_test = pd.read_csv(path_to_csv)
    filtered_df = data_to_test.loc[data_to_test['karta_na_mailing'] == 'BRAK DANYCH']
