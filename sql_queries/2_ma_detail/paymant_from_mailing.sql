select fpp.correspondent_id                               as id_korespondenta,
       sum(fpp.amount)                                    as suma_wplat,
       count(fpp.amount)                                  as liczba_wplat,
       fdagt.text                                         as grupa_akcji_2_wplaty,
       fdagt2.text                                        as grupa_akcji_3_wplaty,
       fcs.name                                           as kod_akcji_wplaty,
       row_number() over (partition by fpp.correspondent_id , fdagt.text, fdagt2.text order by
           fpp.correspondent_id, fdagt.text, fdagt2.text) as numer,
       dzien_po_mailingu
...
group by fpp.correspondent_id, fdagt.text, fdagt2.text, fcs.name, dzien_po_mailingu
