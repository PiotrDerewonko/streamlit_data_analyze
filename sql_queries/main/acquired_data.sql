select date_part('year', adod.data_pozyskania) as rok_dodania, grupa_akcji_1, grupa_akcji_2, kod_akcji,
        ...
        else date_part('month', adod.data_pozyskania)::text end as miesiac_dodania, mailingi, wpłata::text,
    count(adod.correspondent_id) as ilosc
        from fsaps_v_zrodlo_pozyskania_darczyncy adod
        left outer join (
        select distinct correspondent_id , 'dalej w mailingach'::text as mailingi from fsaps_campaign_person
