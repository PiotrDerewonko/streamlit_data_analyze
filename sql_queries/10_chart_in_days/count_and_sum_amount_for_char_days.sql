select fdagt2.text                                          as grupa_akcji_3_wysylki,
       fdagt.text                                           as grupa_akcji_2_wysylki,
       sum(fpp.amount)                                      as suma_wplat,
       fpp.correspondent_id                                 as id_korespondenta,
       count(fpp.amount)                                    as liczba_wplat,
       case when typ is null then 'nowy' else typ::text end as nowy_stary,
       days.dzien_po_mailingu::int
from fsaps_payment_payment fpp
         ...
         left outer join raporty.t_dni_po_nadaniu_mailingow days
                         on days.id_grupy_akcji_3 = fcc.action_group_three_id and
                            days.id_grupy_akcji_2 = fcc.action_group_two_id
                             and days.data_wplywu_srodkow = fpp.date
where fcc.action_group_two_id = #A#
  and fcc.action_group_three_id =#B#
group by grupa_akcji_3_wysylki, grupa_akcji_2_wysylki, id_korespondenta, nowy_stary, days.dzien_po_mailingu
