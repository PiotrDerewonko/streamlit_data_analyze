...
def data_for_sum_of_amount_in_days(mailing, years, days_from, days_to, type_of_operation, data, cumulative, new_old,
                                   choose):
    if type_of_operation == 'sum' or type_of_operation == 'count':
        if days_from > 1:
            data_to_show_not_show = data.loc[(data['dzien_po_mailingu'] <= days_from)]
            data_to_show = data.loc[
                (data['dzien_po_mailingu'] >= days_from + 1) & (data['dzien_po_mailingu'] <= days_to)]
        else:
