...
def add_popover() -> List:
    ...
    if rok_dod:
        with st.container(border=True):
            ...
            filtr_text_rok_dod = f"""(df['rok_dodania'] >= {filtr_rok_dod[0]}) & (df['rok_dodania'] <= {filtr_rok_dod[1]})"""
            if len(filtr_rok_dod) > 0:
    ...
    if przedzial_wieku:
        with st.container(border=True):
            ...
            if len(filtr_przedzial_wieku) > 0:
