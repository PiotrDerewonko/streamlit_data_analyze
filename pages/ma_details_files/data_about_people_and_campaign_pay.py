...
order by łączna_ilość_zamowień desc)a'''
        ...
        sql = '''select fcc.id as id_korespondenta, subaction, case when rodzaj_materialu_pozyskania is null then
    'brak' else rodzaj_materialu_pozyskania end as rodzaj_materialu_pozyskania, case when material_pozyskania is null
        then 'brak' else material_pozyskania end as material_pozyskania, nazwa_kampanii_pozyskania from fsaps_correspondent_correspondent fcc
left outer join (select * from (select foo2.id as id_korespondenta, sourse_id, subaction, date::date, fdago.text as grupa_akcji_1_new,
       case when fdagt.text is not null then fdagt.text else fdagt2.text end as grupa_akcji_2_new,
t.text as grupa_akcji_3_new, f.id as campaign_id, f.topic as nazwa_kampanii_pozyskania
...
left outer join fsaps_dictionary_action_group_three t on f.action_group_three_id = t.id)foo3
 )sub
on sub.id_korespondenta = fcc.id
left outer join (select distinct campaign_id, fmp.value as rodzaj_materialu_pozyskania, fmm.name as material_pozyskania from fsaps_order_order foo
